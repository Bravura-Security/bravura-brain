...
describe('resolveCfAccessConfig', () => {
  test('verified defaults when env unset (opt-in: disabled unless enabled)', () => {
    ...
    // Opt-in: unset → OFF, so an image without the env behaves exactly as before.
    expect(cfg.enabled).toBe(false);
    expect(cfg.groupMap).toEqual({});
  });

  test('enable toggle accepts 1/true/on/yes', () => {
    for (const v of ['1', 'true', 'on', 'yes', 'TRUE', 'On']) {
      expect(resolveCfAccessConfig({ GBRAIN_CF_ACCESS_ENABLED: v } as unknown as NodeJS.ProcessEnv).enabled).toBe(true);
    }
    for (const v of ['', '0', 'false', 'off', 'nope']) {
      expect(resolveCfAccessConfig({ GBRAIN_CF_ACCESS_ENABLED: v } as unknown as NodeJS.ProcessEnv).enabled).toBe(false);
    }
  });

  test('env overrides + enable toggle', () => {
