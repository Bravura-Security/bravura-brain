...
export class PostgresEngine implements BrainEngine {
  ...
  async putPage(slug: string, page: PageInput, opts?: { sourceId?: string }): Promise<Page> {
    ...
    const rows = await sql`
      ...
      ON CONFLICT (source_id, slug) DO UPDATE SET
        ...
        updated_at = now(),
        -- 2026-07 "ghost-write" fix (job 4620): a putPage onto a soft-deleted
        -- row is a restore-on-write — writing fresh content to a page the
        -- caller clearly considers alive must clear deleted_at, otherwise the
        -- update lands invisibly on a row the purge sweep will destroy.
        deleted_at = NULL,
  ...
  async listFactsByEntity(
    ...
    const grep = opts?.grep ? `%${opts.grep}%` : null;
    const rows = await sql<FactRowSqlShape[]>`
      ...
      WHERE source_id = ${source_id}
        ...
        ${grep ? sql`AND fact ILIKE ${grep}` : sql``}
