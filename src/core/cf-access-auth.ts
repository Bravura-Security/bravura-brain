...
export function resolveCfAccessConfig(env: NodeJS.ProcessEnv = process.env): CfAccessConfig {
  ...
  // Opt-IN: the CF Access path is OFF unless explicitly enabled, so an image
  // deployed WITHOUT this env behaves exactly as before (no Cf-Access header
  // verification). Only `1/true/on/yes` turns it on. Pairs with the dual-mode
  // middleware, which also requires the header per request — so even when
  // enabled, header-less requests fall through to the bearer path unchanged.
